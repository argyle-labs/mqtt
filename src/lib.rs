//! mqtt service backend — Mosquitto MQTT broker.
//!
//! Implements `ServiceBackend` so the generic `service.*` tools
//! (deploy/backup/restore/configure/status/connect/sync) drive mqtt. No
...
/// mqtt backend. Holds only the provider name; per-instance endpoint/creds
...
impl ServiceBackend for MqttBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes mqtt can be placed on. `service.deploy` hands the
        ...
        -> BoxFuture<'a, Result<WorkloadSpec, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("mqtt.workload_spec")) })
    }
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("mqtt.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("mqtt.restore")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply mqtt-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("mqtt.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("mqtt.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = MqttBackend::new("mqtt");
        assert_eq!(b.provider(), "mqtt");
    }
}
